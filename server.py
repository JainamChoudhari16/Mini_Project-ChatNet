...
def authenticate_user(client):
    client.send(b"/authenticate")
    username = client.recv(1024).decode("utf-8")
    if username not in user_credentials:
        client.send(b"Authentication failed: Invalid username")
        client.close()
        return None
    password = client.recv(1024).decode("utf-8")
    if password != user_credentials[username]:
        client.send(b"Authentication failed: Invalid password")
        client.close()
        return None
    return username
...
def handshake(client):
    ...
    if username:
        ...
        if groupname in groups:
            # Existing group, handle joining logic here
            if username in groups[groupname].allMembers:
                ...
                groups[groupname].sendMessage(username + " has requested to join the group.", "Chat Room")
                client.send(b"/wait")
...
def main():
    ...
    listenSocket.listen()
    print(f"Server({sys.argv[1]}) is running on port {sys.argv[2]}")
    while True:
